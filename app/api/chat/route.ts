...
export async function POST(req: Request) {
  // Parse request body early - needed for all paths
  ...
  try {
    ...
    if (authError) {
      ...
      return Response.json(
        { error: "Authentication failed", message: "Please try again or refresh the page." },
        { status: 401 },
      );
    }
    ...
    return Response.json(
      { error: "Connection error", message: "Unable to connect to authentication service. Please try again." },
      { status: 503 },
    );
  }

  // PARALLEL: Fetch all needed data at once to minimize pre-stream delay
  const userQuestion = lastUserMessage?.role === "user" ? lastUserMessage.content : "";
  const searchConfig = getAzureSearchConfig();

  if (!searchConfig) {
    return Response.json(
      { error: "Configuration error", message: "Azure AI Search is not configured." },
      { status: 500 },
    );
  }
  ...
  const [
    ...
    // Azure search - all 4 indexes in parallel
  ...
  if (maintSetting?.value === "true" && !profile?.is_admin) {
    ...
    if (isActive) {
      return Response.json(
        { error: "System maintenance", message: msgSetting?.value || "CrimiKnow is currently undergoing scheduled maintenance.", code: "MAINTENANCE" },
        { status: 503 },
      );
    }
  }
  ...
  if (subscription?.current_period_end) {
    ...
    if (new Date() > periodEnd) {
      ...
      if (isFreeTier) {
        ...
        return Response.json(
          { error: "Free trial expired", message: "Your free trial period has expired. Please subscribe to a paid plan.", code: "FREE_TRIAL_EXPIRED" },
          { status: 403 },
        );
      }
      return Response.json(
        { error: "Subscription expired", message: "Your monthly subscription has expired. Please renew your plan.", code: "SUBSCRIPTION_EXPIRED" },
        { status: 403 },
      );
    }
  }
  ...
  if (queriesLimit !== -1 && queriesUsed >= queriesLimit) {
    if (isFreeTier) {
      supabaseAdmin.from("profiles").update({ has_used_free_trial: true }).eq("id", user.id);
      return Response.json(
        { error: "Free trial exhausted", message: "You have used all your free trial questions.", code: "FREE_TRIAL_EXHAUSTED" },
        { status: 403 },
      );
    }
    return Response.json(
      { error: "Query limit reached", message: "You have used all your queries for this billing period.", code: "USAGE_LIMIT_REACHED" },
      { status: 429 },
    );
  }
  ...
  if (lastUserMessage?.role === "user" && curatedMatches && curatedMatches.length > 0) {
    ...
    if (matchedAnswer) {
      // Create session and save messages - fire and forget for speed
      ...
      sessionPromise.then(async ({ data: newSession }) => {
        if (newSession) {
      ...
      return Response.json(
        { content: matchedAnswer.answer, provider: "curated-answer" },
        { headers: { "X-AI-Provider": "curated-answer" } },
      );
    }
  }
  ...
  if (!docsText) {
    return Response.json(
      { error: "No documents found", message: "No relevant documents found. Please try rephrasing your question." },
      { status: 404 },
    );
  }
  ...
  // Create session in background - don't block streaming
  const title = (lastUserMessage?.role === "user" ? lastUserMessage.content : "New Chat").substring(0, 100);

  // Fire off session creation but don't await - we'll use the promise later in flush()
  const sessionPromise = supabaseAdmin.from("chat_sessions").insert({ user_id: user.id, title }).select("id").single();

  // Increment usage in background (fire and forget)
  if (usage) {
...
--- END DOCUMENTS ---`,
    ...
    // Stream using TransformStream - chunks flow immediately to avoid AWS Gateway timeout
    ...
    const transform = new TransformStream<string, Uint8Array>({
      ...
      async flush(controller) {
        try {
          ...
          // Wait for session to be created, then save messages
          ...
          // Clean up old sessions in background (fire and forget)
