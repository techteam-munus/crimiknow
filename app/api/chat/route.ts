...
const getSystemPrompt = async (): Promise<string> => {
  ...
  } catch { /* DB read failed, fall through */ }
  return process.env.AZURE_AI_SYSTEM_PROMPT || 'You are CrimiKnow, an AI-powered criminal law library for Philippine criminal law. Provide comprehensive, detailed answers with legal citations referencing the Revised Penal Code, special penal laws, and relevant jurisprudence.'
}
...
export async function POST(req: Request) {
  ...
  for (const m of messages) {
    agentMessages.push({
      ...
      content: [{ type: 'text', text: m.role === 'user' ? `[Academic legal study question]: ${m.content}` : m.content }]
    })
  }
    ...
    const results = await Promise.allSettled(
      searchIndexes.map(async (indexName) => {
        ...
        const searchResponse = await fetch(searchUrl, {
          ...
          body: JSON.stringify({ search: query, queryType: 'simple', top: 5, select: '*' }),
        })
